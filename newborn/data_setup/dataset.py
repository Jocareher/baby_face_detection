...
class ImageFolderDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        x = self.transform(img) if self.transform is not None else T.ToTensor()(img)
        target = {
            ...
            "child_prob": torch.zeros((0,), dtype=torch.float32),
            "valid_mask": torch.zeros((0,), dtype=torch.bool),
        }
        # Si quieres que el collate pueda devolver batch['paths'] sin tocar export:
        # return {"image": x, "target": target, "path": str(p)}

        return {"image": x, "target": target}
