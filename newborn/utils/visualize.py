...
def get_oriented_face_crop(
    ...
    """
    Extract an oriented face crop directly from the original image.

    This implementation preserves the geometric ratio of the detected OBB
    and avoids forcing the crop to a fixed output size. It also avoids
    synthetic context generation. Instead, it tries to expand the crop by
    the requested scale factor and automatically reduces that factor until
    the full expanded oriented rectangle fits inside the original image.

    Key properties:
        - Uses the original image directly.
        - Preserves the original OBB aspect ratio.
        - Output resolution is proportional to the detected OBB size.
        - Never relies on replicated, white, or black borders to create context.
        - Returns None if even the original OBB is partially outside the image.

    Args:
        base_img: Source image as a NumPy array of shape (H, W, C).
        poly42: Detected OBB polygon as a NumPy array of shape (4, 2).
            Vertex order may be arbitrary.
        desired_scale_crop: Desired multiplicative context factor around
            the detected OBB. A value of 1.0 means no extra context.
            A value of 1.2 means 20% more width and height.
        max_binary_search_steps: Number of iterations used to find the
            largest valid context scale that still fits within the image.
        max_output_side: Optional maximum size for the longest side of the
            ...
            aspect ratio. If None, the crop keeps its natural proportional size.

    Returns:
        An oriented crop as a NumPy array, or None if:
            - the input polygon is invalid,
            - the OBB is degenerate,
            - or the base OBB itself lies partially outside the image.
    """
    ...
    def is_inside_image(points_xy: np.ndarray) -> bool:
        """
        Check whether all points lie inside the valid image domain.
        """
        xs = points_xy[:, 0]
        ys = points_xy[:, 1]
        return bool(
            np.all(xs >= 0.0)
            and np.all(xs <= float(image_width - 1))
            and np.all(ys >= 0.0)
            and np.all(ys <= float(image_height - 1))
        )

    ordered_poly = order_polygon_vertices_tl_tr_br_bl(poly42).astype(np.float32)

    # If the detected OBB itself is already outside the image, there is no
    # clean way to recover missing context without inventing pixels.
    if not is_inside_image(ordered_poly):
        return None

    desired_scale_crop = max(1.0, float(desired_scale_crop))

    # Find the largest valid scale in [1.0, desired_scale_crop] such that
    # the expanded oriented rectangle remains fully inside the image.
    low = 1.0
    ...
    for _ in range(max_binary_search_steps):
        mid = 0.5 * (low + high)
        candidate_rect = _build_scaled_obb_rectangle(ordered_poly, mid)

        if is_inside_image(candidate_rect):
            best_scale = mid
            low = mid
        else:
            high = mid

    src_rect = _build_scaled_obb_rectangle(ordered_poly, best_scale)

    src_width = float(np.linalg.norm(src_rect[1] - src_rect[0]))
    src_height = float(np.linalg.norm(src_rect[3] - src_rect[0]))

    if src_width <= 1e-6 or src_height <= 1e-6:
        return None

    # Preserve the natural crop size derived from the geometry of the
    # expanded OBB in the original image.
    dst_width = max(1, int(round(src_width)))
    dst_height = max(1, int(round(src_height)))

    # Optional safety cap for very large crops.
