...
def _build_scaled_obb_rectangle(
    poly42: np.ndarray,
    scale: float,
) -> np.ndarray:
    """
    Build an expanded oriented rectangle from a 4-point polygon.

    The input polygon is first ordered as top-left, top-right,
    bottom-right, bottom-left. Then a new rectangle is generated by
    scaling its width and height around the polygon center while
    preserving its original orientation.

    Args:
        poly42: Polygon vertices with shape (4, 2).
        scale: Multiplicative factor applied to both width and height.

    Returns:
        Expanded rectangle as a float32 array with shape (4, 2)
        ordered as [TL, TR, BR, BL].
    """
    ordered = order_polygon_vertices_tl_tr_br_bl(poly42).astype(np.float32)
    ...
    if width <= 1e-6 or height <= 1e-6:
        raise ValueError("Degenerate polygon: width or height is too small.")

    width_dir = width_vec / width
    height_dir = height_vec / height

    half_width = 0.5 * width * float(scale)
    half_height = 0.5 * height * float(scale)

    top_left = center - half_width * width_dir - half_height * height_dir
    top_right = center + half_width * width_dir - half_height * height_dir
    bottom_right = center + half_width * width_dir + half_height * height_dir
    bottom_left = center - half_width * width_dir + half_height * height_dir
...
def _build_scaled_obb_rectangle(
    ...
    The polygon is first ordered as top-left, top-right, bottom-right,
    bottom-left. A new rectangle is then generated by scaling its width
    and height around the polygon center while preserving orientation.

    Args:
        poly42: Polygon vertices with shape (4, 2).
        scale: Multiplicative factor applied to width and height.

    Returns:
        Expanded rectangle as a float32 array of shape (4, 2)
        ordered as [TL, TR, BR, BL].
    """
    ordered = order_polygon_vertices_tl_tr_br_bl(poly42).astype(np.float32)
...
def get_oriented_face_crop(
    ...
    poly42 = np.asarray(poly42, dtype=np.float32)
    if poly42.shape != (4, 2):
        raise ValueError(f"Expected poly42 shape (4, 2), got {poly42.shape}.")

    image_height, image_width = base_img.shape[:2]
    ordered_poly = order_polygon_vertices_tl_tr_br_bl(poly42).astype(np.float32)

    width_vec = ordered_poly[1] - ordered_poly[0]
    height_vec = ordered_poly[3] - ordered_poly[0]
    base_width = float(np.linalg.norm(width_vec))
    base_height = float(np.linalg.norm(height_vec))

    if base_width <= 1e-6 or base_height <= 1e-6:
        return None

    angle_deg_ccw = float(math.degrees(float(angle_rad)))

    if pivot == "center":
        pivot_x = float(ordered_poly[:, 0].mean())
        pivot_y = float(ordered_poly[:, 1].mean())
    elif pivot == "tl":
        pivot_x = float(ordered_poly[0, 0])
        pivot_y = float(ordered_poly[0, 1])
    else:
        raise ValueError(f"Unsupported pivot '{pivot}'. Use 'center' or 'tl'.")

    rotation_matrix = cv2.getRotationMatrix2D((pivot_x, pivot_y), angle_deg_ccw, 1.0)

    image_corners = np.array(
        [
            [0.0, 0.0, 1.0],
            [float(image_width), 0.0, 1.0],
            [float(image_width), float(image_height), 1.0],
            [0.0, float(image_height), 1.0],
        ],
        dtype=np.float32,
    ).T

    rotated_corners = (rotation_matrix @ image_corners).T[:, :2]
    min_xy = rotated_corners.min(axis=0)
    max_xy = rotated_corners.max(axis=0)

    translate_x = -float(min_xy[0])
    translate_y = -float(min_xy[1])
    rotation_matrix[:, 2] += [translate_x, translate_y]

    rotated_width = int(math.ceil(float(max_xy[0] - min_xy[0])))
    rotated_height = int(math.ceil(float(max_xy[1] - min_xy[1])))
    ...
    validity_mask = np.ones((image_height, image_width), dtype=np.uint8) * 255
    rotated_validity_mask = cv2.warpAffine(
        validity_mask,
        rotation_matrix,
        (rotated_width, rotated_height),
    ...
    poly_h = np.concatenate(
        [ordered_poly, np.ones((4, 1), dtype=np.float32)],
        axis=1,
    )
    rotated_poly = (rotation_matrix @ poly_h.T).T[:, :2]
    rotated_poly = order_polygon_vertices_tl_tr_br_bl(rotated_poly).astype(np.float32)

    desired_scale_crop = max(1.0, float(desired_scale_crop))

    def rect_is_fully_valid(rect_xy: np.ndarray) -> bool:
        """
        Check whether the expanded rectangle lies fully inside the valid
        rotated support coming from the original image.
        """
        xs = rect_xy[:, 0]
        ys = rect_xy[:, 1]
        ...
        if (
            np.any(xs < 0.0)
            or np.any(xs > float(rotated_width - 1))
            or np.any(ys < 0.0)
            or np.any(ys > float(rotated_height - 1))
        ):
            return False

        x_min = max(0, int(math.floor(float(xs.min()))))
        x_max = min(rotated_width - 1, int(math.ceil(float(xs.max()))))
        y_min = max(0, int(math.floor(float(ys.min()))))
        y_max = min(rotated_height - 1, int(math.ceil(float(ys.max()))))

        if x_max < x_min or y_max < y_min:
            return False

        roi_mask = rotated_validity_mask[y_min : y_max + 1, x_min : x_max + 1]
        if roi_mask.size == 0:
            return False

        rect_local = rect_xy.copy()
        rect_local[:, 0] -= float(x_min)
        rect_local[:, 1] -= float(y_min)

        polygon_mask = np.zeros_like(roi_mask, dtype=np.uint8)
        cv2.fillConvexPoly(
            polygon_mask,
            np.round(rect_local).astype(np.int32),
            255,
        )

        covered_pixels = polygon_mask > 0
        if not np.any(covered_pixels):
            return False

        return bool(np.all(roi_mask[covered_pixels] > 0))

    low = 1.0
    high = desired_scale_crop
    best_scale = 1.0

    for _ in range(max_binary_search_steps):
        mid = 0.5 * (low + high)
        candidate_rect = _build_scaled_obb_rectangle(rotated_poly, mid)

        if rect_is_fully_valid(candidate_rect):
            best_scale = mid
            low = mid
        else:
            high = mid

    final_rect = _build_scaled_obb_rectangle(rotated_poly, best_scale)

    final_width = float(np.linalg.norm(final_rect[1] - final_rect[0]))
    final_height = float(np.linalg.norm(final_rect[3] - final_rect[0]))

    if final_width <= 1e-6 or final_height <= 1e-6:
        return None

    dst_width = max(1, int(round(final_width)))
    dst_height = max(1, int(round(final_height)))

    if max_output_side is not None and max_output_side > 0:
        longest_side = max(dst_width, dst_height)
        if longest_side > max_output_side:
            scale_down = float(max_output_side) / float(longest_side)
            dst_width = max(1, int(round(dst_width * scale_down)))
            dst_height = max(1, int(round(dst_height * scale_down)))
    ...
    perspective_matrix = cv2.getPerspectiveTransform(final_rect, dst_rect)
