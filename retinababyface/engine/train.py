...
def get_base_obb_stats(
    ...
    if (
        obb_stats_by_size
        and resize_size in obb_stats_by_size
        and (base_size is None or base_ratio is None)
    ):
        # Use the precomputed OBB statistics
        stats = obb_stats_by_size[resize_size]
        print(
            f"[INFO] Using precomputed OBB stats for size {resize_size}: size={stats['avg_size']:.2f}, ratio={stats['avg_ratio']:.2f}"
        )
    ...
    if base_size is None or base_ratio is None:
        ...
        print(
            f"[INFO] Computing average OBB statistics from dataset (resize={resize_size})..."
        )
        ...
        print(
            f"[INFO] Computed OBB stats: size={stats['avg_size']:.2f}, ratio={stats['avg_ratio']:.2f}"
        )
        return stats["avg_size"], stats["avg_ratio"]
...
def train(
    ...
    base_size, base_ratio = get_base_obb_stats(
        resize_size, obb_stats_by_size, base_size, base_ratio
    )
