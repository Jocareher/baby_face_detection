...
def get_base_obb_stats(resize_size: Tuple[int, int],
                       obb_stats_by_size: Optional[Dict[Tuple[int, int], Dict[str, float]]],
                       root_dir: str,
                       base_size: Optional[float],
                       base_ratio: Optional[float]) -> Tuple[float, float]:
    """"Adjusts the base size and ratio for the OBB generator based on the dataset statistics.
    Args:
        resize_size (Tuple[int, int]): The size to resize the images to.
        obb_stats_by_size (Optional[Dict[Tuple[int, int], Dict[str, float]]]): Precomputed OBB statistics by size.
        root_dir (str): The root directory of the dataset.
        base_size (Optional[float]): The base size for the OBB generator.
        base_ratio (Optional[float]): The base ratio for the OBB generator.
    Returns:
        Tuple[float, float]: The adjusted base size and ratio.
    """
    # Check if the resize size is in the precomputed OBB statistics
    # and if the base size and ratio are not provided
    if obb_stats_by_size and resize_size in obb_stats_by_size and (base_size is None or base_ratio is None):
        # Use the precomputed OBB statistics
        stats = obb_stats_by_size[resize_size]
        print(f"[INFO] Using precomputed OBB stats for size {resize_size}: size={stats['avg_size']:.2f}, ratio={stats['avg_ratio']:.2f}")
        return stats["avg_size"], stats["avg_ratio"]
    
    # If the base size and ratio are not provided, compute them from the dataset
    if base_size is None or base_ratio is None:
        # Compute the average OBB statistics from the dataset
        # Note: This is a placeholder function, replace with actual implementation
        # that computes the average OBB statistics from the dataset
        print(f"[INFO] Computing average OBB statistics from dataset (resize={resize_size})...")
        raw_dataset = BabyFacesDataset(
            root_dir=root_dir,
            split="train",
            transform=None,
        )
        # Compute the average OBB statistics
        stats = calculate_average_obb_dimensions(raw_dataset, resize_size)
        print(f"[INFO] Computed OBB stats: size={stats['avg_size']:.2f}, ratio={stats['avg_ratio']:.2f}")
        return stats["avg_size"], stats["avg_ratio"]
...
def train(model: nn.Module,
    ...
    base_size, base_ratio = get_base_obb_stats(resize_size, obb_stats_by_size, base_size, base_ratio)
