...
def compute_obb_iou_matrix(
    ...
    (
        ...
        N,
        _,
    ) = anchors.shape  # Get batch size, number of anchors, and coordinates per OBB.
    M = gt_boxes.shape[1]  # Get number of ground truth boxes.
    ious = torch.zeros(
        (B, N, M), dtype=torch.float32
    )  # Initialize IoU matrix with zeros.

    for b in range(B):  # Iterate through each image in the batch.
        for i in range(N):  # Iterate through each anchor box.
            anchor_poly = Polygon(
                anchors[b, i].view(4, 2).cpu().numpy()
            )  # Create Shapely polygon for the anchor.
            ...
            for j in range(M):  # Iterate through each ground truth box.
                gt_poly = Polygon(
                    gt_boxes[b, j].view(4, 2).cpu().numpy()
                )  # Create Shapely polygon for the ground truth.
                if not gt_poly.is_valid:  # Check if the ground truth polygon is valid.
                    continue  # Skip to the next ground truth if not valid.
                inter = anchor_poly.intersection(
                    gt_poly
                ).area  # Calculate intersection area between anchor and ground truth.
                union = anchor_poly.union(
                    gt_poly
                ).area  # Calculate union area between anchor and ground truth.
...
def match_anchors_to_targets(
    ...
    """
    iou_matrix = compute_obb_iou_matrix(
        anchors, gt_boxes
    )  # (B, N, M)  # Calculate IoU matrix.
    ...
    (
        ...
        iou_matrix.shape
    )  # Get batch size, number of anchors, and number of ground truth boxes.

    for b in range(B):  # Iterate through each image in the batch.
        ious_b = iou_matrix[b]  # (N, M)  # Get IoU matrix for the current image.
        max_iou, gt_idx = ious_b.max(
            dim=1
        )  # best match GT for each anchor  # Find best ground truth for each anchor.
        ...
        matched_classes.append(
            gt_classes[b][gt_idx[pos_idx]]
        )  # Store classes associated with positive anchors.
        matched_boxes.append(
            gt_boxes[b][gt_idx[pos_idx]]
        )  # Store OBB targets associated with positive anchors.
        matched_angles.append(
            gt_angles[b][gt_idx[pos_idx]]
        )  # Store angles associated with positive anchors.
